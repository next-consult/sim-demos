    ...
    <div class="row">
        <div class="col-md-12">
            <div class="breadcromb-area">
                <div class="row ">
                    <div class="col-md-4">
                        <h3><span>{{ $devis->client->nom }} </span><span
                                style="font-size: 14px; color: {{ $devis->client->categorie->couleur }};text-transform: initial;"></span>
                        </h3>
                    </div>
                    <div class="col-md-8">
                        <div class="btn-group" role="group" aria-label="Basic example" id="btn_group">
                            ...
                            @if ($devis->status != 'converti_facture' && $devis->status != 'valide')
                                <button type="button" class="btn btn-info  " onclick="savedevis()" id="save_devis"><i
                                        class="fa fa-check"></i>
                                    Enregistrer le
                                    devis</button>
                            @endif
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu pull-right" aria-labelledby="dropdownMenuButton">
                                <ul class="list-group">
                                    ...
                                    @if (count($devis->items) > 0 && $devis->status != 'converti_facture')
                                        <a href="#" onclick="generate_facture()">
                                            <li class="list-group-item"><i class="fa-solid fa-rotate"></i>
                                                Générer une facture</li>
                                        </a>
                                    @endif
                                    @if ($devis->status == 'converti_facture')
                                        <a href="{{ route('factures.update', ['id' => $devis->facture->id]) }}">
                                            <li class="list-group-item"><i class="fa-solid fa-rotate"></i>
                                                Voir facture</li>
                                        </a>
                                    @endif
                                    @if ($devis->bonlivraison == null)
                                        <a href="#" onclick="generate_bonlivraison()">
                                            <li class="list-group-item"><i class="fa-solid fa-rotate"></i>
                                                Générer un bon de livraison</li>
                                        </a>
                                    @else
                                        <a href="{{ url('bonlivraisonsupdate', ['id' => $devis->bonlivraison->id]) }}">
                                            <li class="list-group-item"><i class="fa-solid fa-rotate"></i>
                                                Voir bon de livraison</li>
                                        </a>
                                    @endif
                                </ul>
                            </div>
                        ...
                        <a href="{{ route('devis.index') }} "><button type="button" class="btn btn-warning btn_retour"
                                style=" margin-left: 120px;
"><i class="fa-solid fa-backward"></i>Retour</button></a>

                        @if (count($devis->oportunity) > 0)
                            <a href="{{ route('crm.devis_opp', ['id' => $devis->oportunity[0]->id]) }} "><button
        ...
        <div class="col-md-12">
            <div class="page-box">
                <div class="invoice-box">
                    <h4 class="invoice-status">{{ $devis->status }}</h4>
                    <div class="invoice-head">
                        <h2>#{{ $devis->numero }} @if ($devis->client->type == 'sans_taxe')
                                <span style="font-size: 18px; color: #A93226;text-transform: initial;">(Client exonéré de
                                    tva)</span>
                            @elseif($devis->client->type == 'avec_taxe')
                        ...
                        <div class="row">
                            <div class="col-md-4 ">
                                <div class="invoice-company-address">
                                    <h3>Entreprise</h3>
                                    <p class="name-invoice"><b>{{ $devis->entreprise->nom }}</b></p>
                                    <p>{{ $devis->entreprise->adresse }}</p>
                                    <p>Tel No: {{ $devis->entreprise->telephone }}</p>
                                    <p>Email: {{ $devis->entreprise->email }}</p>
                                    <p>Web: {{ $devis->entreprise->web }}</p>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="billed-to-address">
                                    <h3>Client</h3>
                                    <p class="name-invoice"><b>{{ $devis->client->nom }}
                                        </b>
                                    </p>
                                    <p>{{ $devis->client->adresse }}</p>
                                    <p>Tel No: {{ $devis->client->telephone }}</p>
                                ...
                                <div class="billed-to-address">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-8">
                                                ...
                                                <select class="form-control none" id="client_id">
                                                    @foreach ($clients as $client)
                                                        <option value="{{ $client->id }}"
                                                            {{ $devis->client->id == $client->id ? 'selected' : '' }}>
                                                            {{ $client->nom }}
                                                        </option>
                                                    @endforeach
                                            ...
                                            @else
                                                <span class="form-control disabled">{{ $devis->client->nom }}</span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-8">
                                            {{-- @if ($devis->status != 'converti_facture')
                                            <input type="text" class="form-control" id="numero_devis"
                                                value="{{ $devis->numero }}">
                                            @else
                                            <span class="form-control disabled">{{ $devis->numero }}</span>
                                             @endif --}}
                                            <span class="form-control disabled">{{ $devis->numero }}</span>
                                        ...
                                        <div class="col-md-8">
                                            ...
                                            @if ($devis->status != 'converti_facture')
                                                <input type="date" class="form-control" id="date_devis"
                                                    value="{{ $devis->date }}">
                                            @else
                                                <span class="form-control disabled">{{ $devis->date }}</span>
                                            @endif
                                            ...
                                            @if ($devis->status == 'en cours')
                                                <select class="form-control none" id="status">
                                                    <option value="en cours"
                                                        {{ $devis->status == 'en cours' ? 'selected' : '' }}>En cours
                                                    </option>
                                                    <option value="valide"
                                                        {{ $devis->status == 'valide' ? 'selected' : '' }}>
                                                        Valide</option>
                                                </select>
                                            @elseif($devis->status == 'valide')
                                                <span class="form-control disabled">Valide</span>
                                            @elseif($devis->status == 'converti_facture' || $devis->status == 'valide')
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-8">
                                                ...
                                                <select class="form-control none" id="devise">
                                                    @foreach ($devises as $devv)
                                                        <option value="{{ $devv->symbole }}"
                                                            {{ $devv->symbole == $devis->devise ? 'selected' : '' }}>
                                                            {{ $devv->code }}
                                                        </option>
                                                    @endforeach
                                            ...
                                            @else
                                                <span class="form-control disabled">{{ $devis->devise }}</span>
                                            @endif
                                        </div>
                                    </div>
                    ...
                    <div class="invoice-table">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" id="devis_table">
                                        ...
                                        <tr id="{{ $key + 1 }}">
                                            ...
                                            <td>
                                                <textarea class='form-control' id='description_id[]-{{ $key + 1 }}' name='description[]'
                                                    @if ($devis->status == 'converti_facture' || $devis->status == 'valide') disabled @endif>{{ $item->description }}</textarea>
                                            </td>
                                            <td>
                                                @if ($devis->status != 'converti_facture' && $devis->status != 'valide')
                            ...
                            <div class="col-md-8 col-sm-8">
                                <div class="invoice-note">
                                    ...
                                    {{-- <input style="height:94px" class="form-control" name="condition"
                                        value="@if ($devis->condition) {{ $devis->condition }}@else {{ $devis->entreprise->condition }}@endif" />  --}}


                                    <textarea
                                        class="form-control  @error('condition') is-invalid @enderror @if ($devis->status == 'converti_facture' || $devis->status == 'valide') disabled @endif"
                                        @if ($devis->status == 'converti_facture' || $devis->status == 'valide') disabled @endif name="condition">
                                    @if ($devis->condition)
{{ $devis->condition }}@else{{ $devis->entreprise->condition }}
@endif
                                    </textarea>
                            ...
                            <div class="col-md-4 col-sm-4">
                                <div class="invoice-subtotal">
                                    <p><span>Total HT:</span> <span
                                            id='total_devis_ht'>{{ replace(sprintf('%.3f', $devis->devis_ht)) }}</span>
                                        {{ $devis->devise }}</p>
                                    <p><span>Total Remise:</span> <span
                                            id='total_devis_remise'>-{{ replace(sprintf('%.3f', $devis->devis_remise)) }}</span>
                                        {{ $devis->devise }}
                                    </p>

                                    <p><span>Total TVA:</span> <span
                                            id='total_devis_tva'>{{ replace(sprintf('%.3f', $devis->devis_tva)) }}</span>
                                        {{ $devis->devise }}</p>
                                    <h3 style="font-size:18px">Total TTC:<span
                                            id='total_devis_ttc'>{{ replace(sprintf('%.3f', $devis->devis_ttc)) }}</span>
                                        {{ $devis->devise }} </h3>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="invoice-action">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <textarea class="form-control  @if ($devis->status == 'converti_facture') disabled @endif" name="footer"
                                    @if ($devis->status == 'converti_facture' || $devis->status == 'valide') disabled @endif>
@if ($devis->footer)
{{ $devis->footer }}@else{{ $devis->entreprise->footer }}
@endif
</textarea>
...
<script>
    function test_type() {
        ...
        $("#cataloge_id option").show();

        // Masquer les options qui ne correspondent pas à la valeur de type_produit
        $("#cataloge_id option").each(function() {
    ...
    $(document).ready(function() {
        //status devis
        var status_facture = '{{ $devis->status }}'
        ...
        if (status_facture == 'annuler') {
            ...
            $('.invoice-status').html('Devis valide')
            ...
            $('.invoice-status').html('Devis converti')

        }


        $('#devis_table tr').each(function() {
            if (this.id > 0) {
                var tva_id = "#tva_id-" + this.id

                var type_client = '{{ $devis->client->type }}'
    ...
    function add_catalogue_categorie() {
        ...
        $('#devis_table tr:last').after(
            ...
            " name='description[]'  @if ($devis->status == 'converti_facture' && $devis->status == 'valide') disabled @endif >" +
    ...
    function add_catalogue() {
        ...
        if (cataloge) {
            var page_data = {{ Js::from($devis->items) }};
            var rowCount = $('#devis_table  tr').length;
            var id = 0;
            if (rowCount > 1) {
                var id = $('#devis_table tr').last().attr('id');
    ...
    function savedevis(type_save) {
        ...
        if ($('#devis_table tr').length <= 1) {
            test = false
            title_var = "Le devis est vide"
        ...
        if (test == true) {
            ...
            if (status == "valide") {
                ...
                swal.fire({
                    ...
                    icon: 'question',
                    text: "Etes-vous sûrs,vous voulez valider le devis ??  Vous ne pouvez pas changer le devis ultérieurement !!",
                    type: "warning",
                    showCancelButton: !0,
                        ...
                        jQuery.ajax({
                            ...
                            success: function(result) {
                                ...
                                if (result.error) {
                                    ...
                                    if (result.error.numero_devis) {
                                        error_message(result.error.numero_devis[0], "#numero_devis")
                                    }
                                    if (result.error.date_devis) {
                                        error_message(result.error.date_devis[0], "#date_devis")
                                    }
                                    ...
                                    Swal.fire({
                                        position: 'top-center',
                                        icon: 'success',
                                        title: 'Devis configuré avecc succéss',
    ...
    function total_final() {
        ...
        $("#total_devis_ht").empty()
        $("#total_devis_remise").empty()
        $("#total_devis_tva").empty()
        $("#total_devis_ttc").empty()
        $("#total_devis_ht").html(total_ht.toFixed(3).replace('.', ','))
        $("#total_devis_remise").html('-' + total_remise.toFixed(3).replace('.', ','))
        $("#total_devis_tva").html(total_tva.toFixed(3).replace('.', ','))
        $("#total_devis_ttc").html(total_ttc.toFixed(3).replace('.', ','))
    }

    function test_prix(input) {
        $('#save_devis').attr('disabled', true)
        ...
        jQuery.ajax({
            ...
            success: function(result) {
                ...
                $('#save_devis').attr('disabled', false)
    ...
    function generate_facture() {
        ...
        swal.fire({
            ...
            icon: 'question',
            text: "Etes-vous sûrs,vous voulez générer une facture ??  Vous ne pouvez pas changer le devis ultérieurement !!",
            type: "warning",
            showCancelButton: !0,
            customClass: 'swal-wide',
            confirmButtonText: "Oui, Confirmer!",
            ...
            if (e.value === true) {
                ...
                jQuery.ajax({
                    url: "{{ url('/generatefacture') }}/" + "{{ $devis->id }}",
                    method: 'get',
                    data: {
                        _token: CSRF_TOKEN
                    },
    ...
    function generate_bonlivraison() {
        jQuery.ajax({
            url: "{{ url('/devisbonlivraison') }}/" + "{{ $devis->id }}",
            method: 'post',
            data: {
                _token: "{{ csrf_token() }}",
            },
