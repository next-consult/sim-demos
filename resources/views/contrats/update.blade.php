                ...
                <div class="row">
                    <div class="col-md-7">
                        <div class="seipkon-breadcromb-left">
                            <h3>Modifier le contrat <b>{{ $contrat->numero }}</b></h3>
                        </div>
                    </div>
                    <div class="col-md-5">
    ...
    <div class="row">
                        ...
                        <div class="row">
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label class="control-label">Le client: <span class="obligatoire">*</span></label>
                                    <input type="text" disabled class="form-control" disabled name="client"
                                        value="{{ $contrat->client->nom }}">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    <label class="control-label">L'entreprise: <span class="obligatoire">*</span></label>
                                    <input type="text" disabled class="form-control" disabled name="entreprise"
                                        value="{{ $contrat->entreprise->nom }}">
                                </div>
                            </div>
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <input type="number" placeholder="Nombre de mois" class="form-control" name="nb_mois"
                                         value="{{ $contrat->nb_mois }}">
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
                                ...
                                <select class="form-control none" id="devise">
                                    @foreach ($devises as $devv)
                                        <option value="{{ $devv->symbole }}"
                                            {{ $devv->symbole == $contrat->devise ? 'selected' : '' }}>
                                            {{ $devv->code }}
                                        </option>
                                    @endforeach
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" id="timbre_value"
                                        value="{{ $contrat->facture->timbre }}" onchange="total_final()">
                                </div>
                            </div>
                                ...
                                <div class="invoice-table">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-bordered" id="facture_table">
                                            ...
                                            @if (count($contrat->facture->items) > 0)
                                                @foreach ($contrat->facture->items as $key => $item)
                                                    <tr id="{{ $key + 1 }}">
                                                        ...
                                                        <td>
                                                            ...
                                                            @if (
                                                                $contrat->facture->status != 'paye_partielle' &&
                                                                    $contrat->facture->status != 'valide' &&
                                                                    $contrat->facture->status != 'paye')
                                                        ...
                                                        <td>
                                                            ...
                                                            <textarea class='form-control' id='description_id[]-{{ $key + 1 }}' name='description[]'
                                                                @if (
                                                                    $contrat->facture->status == 'paye_partielle' ||
                                                                        $contrat->facture->status == 'valide' ||
                                                                        $contrat->facture->status == 'paye') disabled @endif>{{ $item->description }}</textarea>
                                                        </td>
                                                        <td>
                                                            @if (
                                                                $contrat->facture->status != 'paye_partielle' &&
                                                                ...
                                                                <input type="number" class="form-control"
                                                                    value="{{ $item->quantites }}" name="quantites[]"
                                                                    min="1" required onchange=calcule($(this))
                                                                    id="quantites_id-{{ $key + 1 }}">
                                                            @else
                                                                <span
                                                                    class="form-control disabled">{{ $item->quantites }}</span>
                                                            @endif
                                                        ...
                                                        <td>
                                                            ...
                                                            @else
                                                                <span
                                                                    class="form-control disabled">{{ $item->prix_ht }}</span>
                                                            @endif
                                                        </td>
                                                        ...
                                                        <td>
                                                            ...
                                                            @else
                                                                <span
                                                                    class="form-control disabled">{{ $item->tva }}</span>
                                                            @endif
                                                        ...
                                                        @if (
                                                            $contrat->facture->status != 'paye_partielle' &&
                                                                ...
                                                                $contrat->facture->status != 'paye')
                                                            <td>
                                                                <a class="btn btn_mobile"
                                                                    onclick="delete_operation($(this))">X
                                                                </a>
                                                            </td>
                                                        @endif
                                                    </tr>
                                        ...
                                        <div class="col-md-11 ">
                                            <div class="invoice-subtotal">
                                                <p><span>Total HT:</span><span id='total_facture_ht'>
                                                        {{ replace(sprintf('%.3f', $contrat->facture->facture_ht)) }}
                                                    </span> {{ $contrat->devise }}
                                                </p>
                                                <p><span>Total Remise:</span>
                                                    <span id='total_facture_remise'>
                                                        {{ replace(sprintf('%.3f', $contrat->facture->facture_remise)) }}


                                                    </span>{{ $contrat->devise }}
                                                </p>
                                                <p><span>Total TVA:</span><span id='total_facture_tva'>
                                                        {{ replace(sprintf('%.3f', $contrat->facture->facture_tva)) }}


                                                    </span> {{ $contrat->devise }}
                                                </p>
                                                <p><span>Timbre:</span><span id='timbre'>
                                                        {{ replace(sprintf('%.3f', $contrat->facture->timbre)) }}
                                                    </span>
                                                    {{ $contrat->devise }}
                                                </p>
                                                <p><span>Total
                                                        TTC:</span> <span class="total-style-ttc" id='total_facture_ttc'>
                                                        {{ replace(sprintf('%.3f', $contrat->facture->facture_ttc)) }}
                                                    </span>{{ $contrat->devise }}
                                                </p>
                                            </div>
                                        </div>
        ...
        <script>
            ...
            function savecontrat() {
                ...
                var nb_mois = $("input[name='nb_mois']").val();
				var date = new Date(date_debut);
                date.setMonth(date.getMonth() + parseInt(nb_mois));
                var date_fin = date.toISOString().slice(0, 10); 
                ...
                if (test == true) {
                    ...
                    jQuery.ajax({
                        ...
                        url: "{{ url('/updatestore') }}/" + '{{ $contrat->id }}',
                        method: 'post',
                        cache: false,
                        contentType: false,
                        processData: false,
                        ...
                        success: function(result) {
                            ...
                            if (result == 200) {
                                Swal.fire({
                                    position: 'top-center',
                                    icon: 'success',
                                    title: 'Contrat modifié avecc succéss',
            ...
            function total_final() {
                ...
                var retenu = 0
                {{-- var items_factures = {{ Js::from($contrat->facture->items) }}; --}}
