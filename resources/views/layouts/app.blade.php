...
<!-- Mirrored from themescare.com/demos/seipkon-admin-template/index.html by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 18 Oct 2022 10:57:52 GMT -->
...
<head>
    ...
    <style>
        ...
        {{-- Table --}} table.table_info td.data {
            background-color: #fde9d9;
        }

        {{-- button groupe --}} @media only screen and (max-width: 768px) {
            /* For mobile phones: */
        ...
        {{-- data table responsive --}} .advance-table,
       ...
       function change_status(status, congeId) {
            fetch(`/changestatus`, {
                ...
                .then(data => {
                    ...
                    } else if (data.error === 'Solde de congé insuffisant') {
                        // Show the user's remaining solde_conge
                        Swal.fire({
                            title: 'Solde de congé insuffisant',
                            text: `Le solde de congé est insuffisant. Cette utilisateur à actuellement ${data.solde_conge} jours de solde restants. Voulez-vous accepter ou refuser le congé quand même ?`,
                            icon: 'warning',
                            showCancelButton: true,
                            allowOutsideClick: false,
                            confirmButtonText: 'Accepter quand même',
                            cancelButtonText: 'Refuser',
                        }).then(result => {
                            if (result.isConfirmed) {
                                // Proceed with acceptance despite insufficient balance
                                fetch(`/changestatus`, {
                                        method: 'POST',
                                        headers: {
                                            'Content-Type': 'application/json',
                                    ...
                                    .then(data => {
                                        if (data.success) {
                                            Swal.fire({
                                                icon: 'success',
                                                title: 'Succès',
                                                text: 'Le statut du congé a été mis à jour avec succès malgré le solde insuffisant.'
                                            }).then(() => {
                                                location.reload();
                                            });
                                        } else {
...
<!-- Mirrored from themescare.com/demos/seipkon-admin-template/index.html by HTTrack Website Copier/3.x [XR&CO'2014], Tue, 18 Oct 2022 10:58:30 GMT -->
