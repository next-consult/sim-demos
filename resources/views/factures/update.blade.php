                ...
                <div class="row ">
                    <div class="col-md-4">
                        @if ($facture->type == 'client')
                            <h3><span>{{ $facture->client->nom }} </span></h3>
                        @endif
                        @if ($facture->type == 'fournisseur')
                            <h3><span>{{ $facture->fournisseur->nom }} </span></h3>
                        @endif
                    </div>
                    ...
                    <div class="col-md-8">
                        <div class="btn-group" role="group" aria-label="Basic example" id="btn_group">
                            ...
                            @if ($facture->status != 'paye_partielle' && $facture->status != 'valide' && $facture->status != 'paye')
                                <button type="button" class="btn btn-info  " onclick="savefacture()" id="save_facture"><i
                                        class="fa fa-check"></i>
                                    Enregistrer la
                                    facture</button>
                            @endif
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu pull-right" aria-labelledby="dropdownMenuButton">
                                    <ul class="list-group">
                                        ...
                                        @if (count($facture->contrat) == 0)
                                            <a href="#" data-toggle="modal" data-target="#contrat_lie">
                                        ...
                                        @else
                                            <a href="#" data-toggle="modal" data-target="#contrat_lie">
                                                <li class="list-group-item"><i class="fa fa-file"
                                                        style="margin-right:5px"></i>
                                                    Contrat ({{ $facture->contrat[0]->numero }})</li>
                                            </a>
                                        @endif



                                        @if ($facture->facture_ttc > 1000)
                                            <a href="{{ route('factures.retenu', ['id' => $facture->id]) }}">
                                                <li class="list-group-item"><i class="fa fa-file"
                                                        style="margin-right:5px"></i>
                                                    Retenu a la source </li>
                                            </a>
                                        @endif
                            ...
                            <a
                                href="@if ($facture->type == 'client') {{ route('factures.index') }} @else{{ route('depense.index') }} @endif "><button
                                    type="button" class="btn btn-warning btn_retour" style=" margin-left: 120px;"><i
                                        class="fa-solid fa-backward"></i>Retour</button></a>
                            @if (count($facture->oportunity) > 0)
                                <a href="{{ route('crm.devis_opp', ['id' => $facture->oportunity[0]->id]) }} "><button
                                        type="button" class="btn btn-success " style=" margin-left: 120px;
">Opportunité <i
                                            class="fa-solid fa-chart-simple"></i></button></a>
                            @endif
                        </div>
                    ...
                    <div class="invoice-head">
                        <h2>#{{ $facture->numero }}
                            @if ($facture->client_id)
                                <input type="hidden" value="{{ $facture->client->type }}" id="type_client" />
                                @if ($facture->client->type == 'sans_taxe')
                                    <span style="font-size: 18px; color: #A93226;text-transform: initial;">(Client exonéré
                                        de tva)</span>
                                @elseif($facture->client->type == 'avec_taxe')
                            ...
                            <div class="col-md-2">
                                <div class="billed-to-address">
                                    ...
                                    @elseif($facture->fournisseur_id)
                                        <h3>Fournisseur</h3>
                                        <p class="name-invoice"><b>{{ $facture->fournisseur->nom }}
                                            </b>
                                        </p>
                                        <p>{{ $facture->fournisseur->adresse }}</p>
                                        <p>Tel No: {{ $facture->fournisseur->telephone }}</p>
                                        <p>Email: {{ $facture->fournisseur->email }}</p>
                                    @endif
                                </div>
                            </div>
                            <div class="col-md-4 ">
                                <div class="billed-to-address">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-7">
                                                ...
                                                <select class="form-control none" id="client_id">
                                                    @foreach ($clients as $client)
                                                        <option value="{{ $client->id }}"
                                                            {{ $facture->client->id == $client->id ? 'selected' : '' }}>
                                                            {{ $client->nom }}
                                                        </option>
                                                    @endforeach
											...
											@endif
                                            @else
                                                <span class="form-control disabled">{{ $facture->client->nom }}</span>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-7">
                                            ...
                                            @if ($facture->status == 'en cours')
                                                <select class="form-control none" id="status">
                                                    ...
                                                    <option value="valide"
                                                        {{ $facture->status == 'valide' ? 'selected' : '' }}>
                                                        Valide</option>
                                                </select>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-7">
                                                ...
                                                <select class="form-control none" id="devise">
                                                    @foreach ($devises as $devv)
                                                        <option value="{{ $devv->symbole }}"
                                                            {{ $devv->symbole == $facture->devise ? 'selected' : '' }}>
                                                            {{ $devv->code }}
                                                        </option>
                                                    @endforeach
                                            ...
                                            @else
                                                <span class="form-control disabled">{{ $facture->devise }}</span>
                                            @endif
                                        </div>
                                    </div>
                                        ...
                                        <div class="col-md-7">
                                           ...
                                           <input type="date" class="form-control" id="date_facture"
                                            ...
                                            <input @if ($facture->status == 'paye' || $facture->status == 'paye_partielle' || $facture->status == 'valide') disabled @endif type="number"
                                                class="form-control" id="timbre_value" value="{{ $facture->timbre }}"
                                                onchange="total_final()">

                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <p class="name-invoice col-md-5 col-form-label" style="margin-top:10px"><b>Retenu
                                                à la source (%)</b></p>
                                        <div class="col-md-7">
                                        ...
                                        <tbody>
                                            @foreach ($facture->frais as $frais)
                                                <tr>
                                                    <td style="width:80%">
                                                        <br>
                                                        <input @if ($facture->status == 'paye' || $facture->status == 'paye_partielle' || $facture->status == 'valide') disabled @endif
                                                            type="hidden" class="form-control"
                                                            placeholder="Nom du frais" name="id_frais[]"
                                                            value="{{ $frais->id }}">

                                                        <input @if ($facture->status == 'paye' || $facture->status == 'paye_partielle' || $facture->status == 'valide') disabled @endif
                                                            type="text" class="form-control"
                                                            placeholder="Nom du frais" name="titre[]"
                                                            value="{{ $frais->nom }}">
                                                        <input @if ($facture->status == 'paye' || $facture->status == 'paye_partielle' || $facture->status == 'valide') disabled @endif
                                                            type="number" class="form-control" name="montant[]"
                                                            onchange='total_final()' placeholder="Montant"
                                                            style="margin-top:5px" value="{{ $frais->montant }}">
                                                    </td>
                    ...
                    <div class="invoice-table">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered" id="facture_table">
                                ...
                                <input type="hidden" id="test_quantites" value=true />
                                        ...
                                        <tr id="{{ $key + 1 }}">
                                            <td>
                                                <input type='hidden' class='form-control' value={{ $item->id }}
                                                    name='id[]'>
                                                <input type='hidden' class='form-control' value={{ $item->categorie }} 
                                            ...
                                            <td>
                                                <textarea class='form-control' id='description_id[]-{{ $key + 1 }}' name='description[]'
                                                    @if ($facture->status == 'paye_partielle' || $facture->status == 'valide' || $facture->status == 'paye') disabled @endif>{{ $item->description }}</textarea>
                                            </td>
                                            <td>
                                                @if ($facture->status != 'paye_partielle' && $facture->status != 'valide' && $facture->status != 'paye')
                                                    <input type="number" class="form-control"
                                                        ...
                                                        data-value="{{ $item->categorie }}"
                                                        onkeyup="test_quantites($(this))"
                    ...
                    <div class="invoice-footer-note">
                        <div class="row">
                            <div class="col-md-8">
                                ...
                                @endif
                                @if (count($catalogues_vendu) > 0)
                                    <div class="invoice-company-address">
                                        <h3>Les clés oem vendus</h3>
                                    </div>
                                    <table class="table table-bordered" style="width:80%;margin-top:10px">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            @foreach ($catalogues_vendu as $cle)
                                                <tr>
                                                    <td class="livraison">
                                                        {{ $cle->numero_cle }}
                                                    </td>
                            ...
                            <div class="col-md-4 col-sm-4">
                                <div class="invoice-subtotal">
                                    <p><span>Total HT:</span><span id='total_facture_ht'>
                                            {{ replace(sprintf('%.3f', $facture->facture_ht)) }} </span>
                                        {{ $facture->devise }}</p>
                                    <p><span>Total Remise:</span>
                                        <span
                                            id='total_facture_remise'>-{{ replace(sprintf('%.3f', $facture->facture_remise)) }}</span>{{ $facture->devise }}
                                    </p>


                                    <p><span>Total TVA:</span><span id='total_facture_tva'>
                                            {{ replace(sprintf('%.3f', $facture->facture_tva)) }}</span>
                                        {{ $facture->devise }}
                                    </p>
                                    ...
                                    @if ($facture->retenu > 0)
                                        <p><span>Retenu a la source:</span><span id='total_retenu_ht'>
                                                -{{ replace(sprintf('%.3f', $facture->facture_retenu)) }}</span>
                                            {{ $facture->devise }}
                                        </p>
                                    @endif
                            ...
                            <div class="col-md-8 col-sm-8">
                                <div class="invoice-note">
                                    ...
                                    <textarea class="form-control  @error('condition') is-invalid @enderror "
                                        @if ($facture->status == 'paye_partielle' || $facture->status == 'valide' || $facture->status == 'paye' ) disabled @endif name="condition">
@if ($facture->condition)
{{ $facture->condition }}@else{{ $facture->entreprise->condition }}
@endif
</textarea>
                    ...
                    <div class="invoice-action">
                        <div class="row">
                            <div class="col-md-12">
                                ...
                                <textarea class="form-control" name="footer" @if ($facture->status == 'paye_partielle' || $facture->status == 'valide' || $facture->status == 'paye') disabled @endif>
@if ($facture->footer)
{{ $facture->footer }}@else{{ $facture->entreprise->footer }}
@endif
</textarea>
    ...
    <div class="modal fade" id="active_retenu" role="dialog" aria-labelledby="exampleModalCenterTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <label for="inputPassword" class="col-md-4 col-form-label">Entrer
                            le retenu</label>
                        <div class="col-md-8">
                            <input type="hidden" id="client_id" />
                            <input type="number" class="form-control " id="valeur_retenu" value="1" />
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary" onclick="add_retenue()">Ajouter</button>
                </div>
    ...
    <div class="modal fade" id="produit_catalogue" role="dialog" aria-labelledby="exampleModalCenterTitle"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-8">
                            ...
                            <select class="form-control"style="width: 100%" id="type_produit" onchange="test_type()">
                                <option value="Clés_OEM" style="float:left" selected>
                                    Clés OEM
                                </option>
                                {{-- <option value="Licences_ANTIVIRUS" style="float:left">
                                    Licences ANTIVIRUS
                                </option>
                                <option value="Equipements_HARDWARE" style="float:left">
                                    Equipements HARDWARE
                                </option>
                                <option value="Services_Prestations" style="float:left">
                            ...
                            <select class="js-example-basic-single js-states form-control"style="width: 100%"
                                ...
                                @foreach ($catalogues as $catalogue)
                                    <option value="{{ $catalogue->id }}" style="float:left">
                                        {{ $catalogue->produit }} ( {{ $catalogue->categorie }})</option>
                                @endforeach
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-warning" data-dismiss="modal">Retour</button>
                    <button type="submit" class="btn btn-primary" onclick="add_catalogue_categorie()">Ajouter</button>
                </div>
    ...
    <div class="modal fade" id="contrat_lie" role="dialog" aria-labelledby="exampleModalCenterTitle"
        aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form id="paiement_form">
                    ...
                    <div class="modal-body">
                        <div class="form-group row">
                            <label for="inputPassword" class="col-md-4 col-form-label">
                                Choisir le contrat</label>
                            <div class="col-md-8">
                                <select id='contrat_id'>
                                    <option value='sans_contrat'>
                                        Sans contrat</option>
                                    @foreach ($contrats as $contrat)
                                        <option
                                            value='{{ $contrat->id }}'@if (count($facture->contrat) > 0) {{ $facture->contrat[0]->id == $contrat->id ? 'selected' : '' }} @endif>
                                            {{ $contrat->numero }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary" id="submit_paiement" onclick="lie_contrat()">Lié
                            le contrat</button>
                    </div>
                </form>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            <label for="inputPassword" class="col-md-4 col-form-label">
                                Montant</label>
                            <div class="col-md-8">
                                <input type="hidden" id="facture_id" value="{{ $facture->id }}" />
...
<script>
    function test_quantites(input) {
        $('.erreur').empty();

        var categorie = input.attr('data-value');
        console.log(categorie)
        if (categorie == "oem") {
            let quantite_val = input.val()
            $.ajaxSetup({
                headers: {
            ...
            jQuery.ajax({
                url: "{{ url('/test_cles') }}/" + quantite_val + "/{{ $facture->id }}",
                method: 'get',
                success: function(result) {
                    $('.erreur').empty();
                    if (result.erreur_stock) {
                        let message =
                            `<p class='text-danger erreur' style='float:left;font-size:13px' >${result.message}</p>`
                        $(message).insertAfter(input);
                        input.val(result.quantites_restant)
                        $('#test_quantites').val(false)

                    } else {
                        $('#test_quantites').val(true)
...
<script>
    ...
    function lie_contrat() {
        let facture_id = '{{ $facture->id }}'
        ...
        jQuery.ajax({
            ...
            success: function(result) {
                if (result.error) {
                    ...
                    if (result.error.valeur_retenu) {
                        error_message(result.error.valeur_retenu[0], "#contrat_id")
                    }
                    ...
                    Swal.fire({
                        position: 'top-center',
                        icon: 'success',
                        title: 'Facture lié avec succéss',
    ...
    function add_retenue() {
        $('.erreur').empty()
        $('#submit_active_retenu').attr('disabled', 'disabled');
        $.ajaxSetup({
            headers: {
                'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
            }
        });
        var valeur_retenu = jQuery('#valeur_retenu').val()
        jQuery.ajax({
            url: "{{ url('/activeretenufacture') }}/" + '{{ $facture->id }}',
            method: 'get',
            data: {
                valeur_retenu: valeur_retenu,
                _token: "{{ csrf_token() }}",
            },
            success: function(result) {
                ...
                if (result.error) {
                    ...
                    $('#submit_active_retenu').removeAttr('disabled');

                    if (result.error.valeur_retenu) {
                        error_message(result.error.valeur_retenu[0], "#valeur_retenu")
                    }
                    ...
                    Swal.fire({
                        position: 'top-center',
                        icon: 'success',
                        title: 'Retenu a la source activé avec succéss',
                        showConfirmButton: false,
                        timer: 1000
                    })
                            ...
                            success: function(result) {
                                ...
                                if (result.error) {
                                    ...
                                    if (result.error.numero_facture) {
                                        error_message(result.error.numero_facture[0],
                                            "#numero_facture")
                                    }
        ...
        jQuery.ajax({
            ...
            success: function(result) {
                ...
                $('#facture_table tr:last').after(
                    ...
                    " name='description[]'  @if ($facture->status == 'converti_facture' && $facture->status == 'valide') disabled @endif >" +
                    "" + result.description + "</textarea></td>" +
                    "<td><input type='number' class='form-control'  value='" + result.quantites +
                    "' name='quantites[]' onkeyup='test_quantites($(this))' data-type='default' onchange=calcule($(this)) required id=" +
                    quantites_id +
                    ...
                    result.remise + "' name='remise[]'  onchange=calcule($(this))  id=" + remise_id +
                    ...
                    "><option value='pourcentage' " + (result.type_remise == 'pourcentage' ?
                        'selected' : '') +
                    "><span style='font-size:10px'>%</span></option><option value='montant' " + (result
                        .type_remise == 'montant' ? 'selected' : '') +
    ...
    function addoperation(test) {
        ...
        $('#facture_table tr:last').after(
            "<tr id=" + id_1 +
            "><td> <input type='hidden' class='form-control' value='default' name='categorie[]'> <input type='hidden' class='form-control' value='new' name='id[]' > <select onchange=catalogue_info($(this).val(),"+catalogue_id.toString()+") class='js-example-basic-single js-states form-control' style='width: 100%'"+
                            ...
                            "</textarea></td>"+
                            "<td><input type='number' class='form-control' value='default' onkeyup='test_quantites($(this))' name='quantites[]' onchange=calcule($(this)) required id="+quantites_id+"></td><td> <input type='text' onchange=calcule($(this));test_prix($(this))  class='form-control' value='' name='prix_ht[]' old-value='' required id="+prix_id+"></td>"+
                            "<td> <select onchange=calcule($(this))  name='tva[]'class='form-control none' style='margin-top:5px' id=" +
            tva_id +
            ">"+
                            "@foreach ($taxes as $taxe)"+
        ...
        $('#facture_table tr:last').after(
            "<tr id=" + id_1 +
            "><td> <input type='hidden' class='form-control' value='default' name='categorie[]'> <input type='hidden' class='form-control' value='new' name='id[]'>  <input type='text' class='form-control' name='produits[]' id=" +
            catalogue_id + "_produit>" +
            "</td>" +
            "<td><textarea class='form-control' id=" + description +
            " name='description[]' @if ($facture->status == 'paye_partielle' && $facture->status == 'valide') disabled @endif >" +
            "</textarea></td>" +
            "<td><input type='number' class='form-control' value='' name='quantites[]' onkeyup='test_quantites($(this))' data-value='default' onchange=calcule($(this)) required id=" +
            quantites_id +
            "></td><td> <input type='text' onchange=calcule($(this));test_prix($(this))  class='form-control' value='' name='prix_ht[]' old-value='' required id=" +
            prix_id + "></td>" +
            "<td> <select onchange=calcule($(this))  name='tva[]'class='form-control none' style='margin-top:5px' id=" +
            tva_id +
            ">" +
    ...
    function add_catalogue_categorie() {
        var result = {{ Js::from($parametre) }};
        var test_quantites = {{ Js::from($test_quantites) }};

        var count = $("input[name='quantites[]'][data-value='oem']").length;
        if (count != 0) {
            Swal.fire({
                position: 'top-center',
                icon: 'error',
                title: 'Vous avez une ligne qui contient des clés oems',
                showConfirmButton: true,
            })
            return false
        }
        if (test_quantites == 0) {
            let element =
                `<a href="{{ route('catalogues.add') }}"><button type="button" class="btn btn-warning btn_retour" style="margin-left: 120px;"><i class="fa-solid fa-backward"></i>Ajouter</button></a>`;

            Swal.fire({
                position: 'top-center',
                icon: 'error',
                title: `Il n'y a pas de clés dans le stock`,
                showConfirmButton: true,
            });
            return false
        }
        var rowCount = $('#devis_table  tr').length;
        var id = 0;
        if (rowCount > 1) {
            var id = $('#devis_table tr').last().attr('id');
        }
        ...
        let final = total(result.prix_ht, 1, result.tva, 0, 'pourcentage')
        console.log(final)
        $('#facture_table tr:last').after(
            "<tr id=" + id_1 +
            "><td><input type='hidden' class='form-control' value='new' name='id[]'><input type='hidden' class='form-control' value='oem' name='categorie[]'>  <input type='text' class='form-control' name='produits[]' value='" +
            result.produit + "' id=" + catalogue_id + "_produit>" +
            "</td>" +
            "<td><textarea class='form-control' id=" + description +
            " name='description[]'  @if ($facture->status == 'converti_facture' && $facture->status == 'valide') disabled @endif >" +
            "" + result.description + "</textarea></td>" +
            "<td><input type='number' class='form-control'  value='" + result.quantites +
            "' name='quantites[]' onkeyup='test_quantites($(this))' data-value='oem' onchange=calcule($(this)) required id=" +
            quantites_id +
            "></td><td> <input type='text' onchange=calcule($(this));test_prix($(this))  class='form-control'  name='prix_ht[]' value='" +
            result.prix_ht + "' old- required id=" + prix_id + "></td>" +
            "<td> <select onchange=calcule($(this))  name='tva[]'class='form-control none' style='margin-top:5px' id=" +
            ...
            "><span style='font-size:10px'>%</span></option><option value='montant' " + (result
                .type_remise == 'montant' ? 'selected' : '') +
            "><span style='font-size:10px'>TND </span></option></select></div></div></td><td id=" +
            total_ht_id + ">" + final.total_ht + "</td><td id=" + total_remise_id + ">" +
            final.total_remise + "</td><td id=" + total_tva_id + ">" + final.total_tva +
            "</td><td id=" + total_ttc_id + ">" + final.total_ttc +
            "</td><td><a class='btn btn_mobile' onclick ='delete_operation($(this))'>X</a></td></tr>"
        );
    ...
    function total_final() {
        ...
        if ('{{ $facture->facture_ttc }}' >= 1000) {
            montant_retenu = ('{{ $facture->retenu }}' / 100)
            retenu = total_ttc * montant_retenu
            total_ttc -= retenu
